...
#testing individual new images added
"""
results3 = model.predict(source=os.path.join('Yolo_data', 'test', 'images', 'phone_02.jpg'), conf=0.36, save=False)
for r in results3:
    for box in r.boxes:
        class_id = int(box.cls)
        confidence = box.conf[0]
        name = model.names[class_id]
        print(f"Detected: {name} (ID: {class_id}) with {confidence:.2f} confidence")
"""
...
sample_size = min(len(all_test_images), 70)
...
results2 = model.predict(source=random_sample_paths, conf=0.36, save=False, augment=True)
...
with open(csv_file, 'w', newline='') as f:
    ...
    for cls_id, quantity in prediction_summary.items():
        multiplier = multipliers.get(cls_id, 1)
        scaled_quantity = quantity * multiplier
        writer.writerow([cls_id, scaled_quantity, current_date, '']) # use class id as stock code
